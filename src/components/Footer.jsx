import React, { useState, useEffect } from 'react'
import { motion } from 'framer-motion'
import footerData from '../data/footer.json'

const ScrollButton = () => {
  const [isAtTop, setIsAtTop] = useState(true)

  useEffect(() => {
    const handleScroll = () => {
      setIsAtTop(window.scrollY < 100)
    }

    window.addEventListener('scroll', handleScroll)
    return () => window.removeEventListener('scroll', handleScroll)
  }, [])

  const handleClick = () => {
    if (isAtTop) {
      window.scrollTo({ top: document.documentElement.scrollHeight, behavior: 'smooth' })
    } else {
      window.scrollTo({ top: 0, behavior: 'smooth' })
    }
  }

  return (
    <motion.button
      onClick={handleClick}
      className="fixed bottom-8 inset-x-0 mx-auto flex justify-center p-3 glass rounded-full text-slate-400 
                 hover:text-accent hover:bg-white/10 transition-all duration-300 z-50 w-fit"
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
      whileHover={{ scale: 1.1 }}
      whileTap={{ scale: 0.95 }}
    >
      {isAtTop ? (
        // Mouse scroll indicator
        <div className="w-5 h-8 border-2 border-current rounded-full flex justify-center pt-1.5">
          <motion.div
            className="w-1 h-2 bg-current rounded-full"
            animate={{ y: [0, 6, 0], opacity: [1, 0.3, 1] }}
            transition={{ duration: 1.5, repeat: Infinity, ease: "easeInOut" }}
          />
        </div>
      ) : (
        // Up arrow
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 10l7-7m0 0l7 7m-7-7v18" />
        </svg>
      )}
    </motion.button>
  )
}

const Footer = () => {
  const currentYear = new Date().getFullYear()
  const socialLinks = footerData.socialLinks.map(link => ({
    ...link,
    icon: link.name === 'GitHub' ? (
      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
        <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"/>
      </svg>
    ) : link.name === 'LinkedIn' ? (
      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
        <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/>
      </svg>
    ) : (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
      </svg>
    )
  }))

  return (
    <footer className="py-12 px-6 mt-20">
      <div className="max-w-6xl mx-auto">
        {/* Divider */}
        <div className="h-px bg-gradient-to-r from-transparent via-slate-700 to-transparent mb-12" />

        {/* Social Links - Mobile */}
        <div className="flex justify-center gap-6 mb-8 md:hidden">
          {socialLinks.map((social) => (
            <motion.a
              key={social.name}
              href={social.url}
              target="_blank"
              rel="noopener noreferrer"
              className="p-3 glass rounded-full text-slate-400 hover:text-slate-200 transition-colors"
              whileHover={{ scale: 1.1, y: -2 }}
              whileTap={{ scale: 0.95 }}
            >
              {social.icon}
            </motion.a>
          ))}
        </div>

        {/* Main Footer Content */}
        <div className="text-center space-y-4">
          <motion.p
            className="text-sm text-slate-500"
            initial={{ opacity: 0 }}
            whileInView={{ opacity: 1 }}
            transition={{ duration: 0.5 }}
          >
            Designed & Built by{' '}
            <a
              href="https://github.com/Alan0893"
              target="_blank"
              rel="noopener noreferrer"
              className="text-slate-300 hover:text-slate-100 transition-colors"
            >
              Alan Lin
            </a>
          </motion.p>

          <motion.p
            className="text-xs text-slate-600 max-w-md mx-auto leading-relaxed"
            initial={{ opacity: 0 }}
            whileInView={{ opacity: 1 }}
            transition={{ duration: 0.5, delay: 0.1 }}
          >
            Built with{' '}
            {footerData.footer.builtWith.map((tech, idx) => (
              <React.Fragment key={tech}>
                <span className="text-slate-500">{tech}</span>
                {idx < footerData.footer.builtWith.length - 1 && ', '}
              </React.Fragment>
            ))}
          </motion.p>

          <motion.p
            className="text-xs text-slate-700 font-mono"
            initial={{ opacity: 0 }}
            whileInView={{ opacity: 1 }}
            transition={{ duration: 0.5, delay: 0.2 }}
          >
            © {currentYear} Alan Lin. All rights reserved.
          </motion.p>
        </div>

        {/* Scroll button - centered at bottom */}
        <ScrollButton />
      </div>
    </footer>
  )
}

export default Footer
